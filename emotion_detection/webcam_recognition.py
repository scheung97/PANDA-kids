...
NEGATIVE_INDEX = EXPRESSIONS.index('Negative')
MIN_NEGATIVE_FRAMES = 350
NUM_SAVED_FRAMES = 700
emotion_history = deque(maxlen=NUM_SAVED_FRAMES)
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        if show_video:
            frame = cv2.putText(frame, EXPRESSIONS[prediction] + ": {}".format(emotion_history.count(NEGATIVE_INDEX)), (x, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
            frame = cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)

    emotion_history.extend(face_predictions)

    if emotion_history.count(NEGATIVE_INDEX) > MIN_NEGATIVE_FRAMES:
        print("Publishing to ui channel")
